...
const messages = stylelint.utils.ruleMessages(
  ...
  {
    expectedClassName: (className, filePath) =>
      `Unable to find composed "${className}" class in ${filePath}`,
    expectedFileExists: (filePath) => `No file found at this path: ${filePath}`,
    expectedIsFile: (filePath) =>
      ...
      if (filePath[0] === ".") {
        ...
        if (rootAlias) {
          resolvedFilePath = path.join(
            aliasMap[rootAlias],
            ...resolvedFilePath.split(path.sep).slice(1)
          );
    ...
    return (root, result) => {
      ...
      root.walkDecls((decl) => {
        if (decl.prop === "composes") {
          ...
          const classNames = expressions[0]
            .replace(/\s+/g, " ")
            .trim()
            .split(" ");
          const fromExpression = expressions[1]
            ? expressions[1].trim()
            : `'${result.opts.from}'`;
          ...
          const fromFilePath = resolveFilePath(
            contextPath,
            fromExpression.slice(1, -1)
          );
          ...
          classNames
            ...
            .filter(
              (className) =>
                !RegExp(`\\.${className}[\\s\\.,:{'"]`).test(fileContents)
            )
            .forEach((className) =>
              stylelint.utils.report({
                index: decl.lastEach,
